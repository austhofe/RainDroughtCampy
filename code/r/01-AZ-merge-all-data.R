...
az_county_info <- read_excel("data/01_az_county-info.xlsx")

#### Import and Export CO Campy and Salmonella Data ####
health <- read_excel("data/HealthData/AZ2009-2020.xlsx")
...
health %>%
  mutate(campycount = case_when(Pathogen == 'CMP' ~1,
                                Pathogen == 'SLM' ~ 0))
health %>%
  mutate(salcount = case_when(Pathogen == 'CMP' ~ 0,
                              Pathogen == 'SLM' ~ 1))
...
clean_campy_data <- healthdf_campy %>% 
  left_join(az_county_info, by=c("county"))
...
raw_prism_files <- data_frame(filename = list.files('data/PRISMData/AZDailyObserved/'))

raw_prism_file_paths <- raw_prism_files  %>%
  mutate(filepath = paste0("data/PRISMData/AZDailyObserved/", filename))
...
clean_prism_data <- raw_prism_data %>% 
  left_join(az_county_info, by=c("lat","long"))
...
# Import the file names so that you can loop through them
raw_prism30_files <- data_frame(filename = list.files('data/PRISMData/AZ30YearNormals/'))

raw_prism30_file_paths <- raw_prism30_files  %>%
  mutate(filepath = paste0("data/PRISMData/AZ30YearNormals/", filename))
...
raw_SoVI_2010 <- read_csv("data/SoVI/Arizona_CT2010.csv")
SoVI_2010 <- raw_SoVI_2010[!(raw_SoVI_2010$STATE_NAME=="4" | raw_SoVI_2010$R_PL_THEMES=="-999"),]
...
raw_SoVI_2014 <- read_csv("data/SoVI/Arizona_COUNTY2014.csv")
clean_SoVI_2014 <- raw_SoVI_2014 %>%
  select(FIPS,RPL_THEMES)
clean_SoVI_2014$year <- "2014"

raw_SoVI_2016 <- read_csv("data/SoVI/Arizona_COUNTY2016.csv")
clean_SoVI_2016 <- raw_SoVI_2016 %>%
  select(FIPS,RPL_THEMES)
clean_SoVI_2016$year <- "2016"

raw_SoVI_2018 <- read_csv("data/SoVI/Arizona_COUNTY2018.csv")
...
# PDSI
# Import the file names so that you can loop through them
raw_pdsi_files <- data_frame(filename = list.files('data/NOAA/NCEI/AZ/', pattern = '-pdsi'))

raw_pdsi_file_paths <- raw_pdsi_files  %>%
  mutate(filepath = paste0("data/NOAA/NCEI/AZ/", filename))
...
#Z Index
# Import the file names so that you can loop through them
raw_zndx_files <- data_frame(filename = list.files('data/NOAA/NCEI/AZ/', pattern = '-zndx'))
...
final_az_df_analysis <- filter(final_az_df, year>=2009)

final_az_df_analysis$enso <- ifelse(final_az_df_analysis$year==2006 | final_az_df_analysis$year==2007 | final_az_df_analysis$year==2009 | final_az_df_analysis$year==2010 | final_az_df_analysis$year==2014 | final_az_df_analysis$year==2015 | final_az_df_analysis$year==2016 | final_az_df_analysis$year==2018 | final_az_df_analysis$year==2019,1,0)


#### EXPORTING DATA FRAME ####
write.csv(final_az_df_analysis, "data\\final_az_df_analysis.csv", row.names = T)

##To DO
# Compare daily precip to avg then compute a yes/no
# Do the same thing but for weekly and month