...
nm_county_info <- read_excel("data/01_nm_county-info.xlsx")

#### Import and Export CO Campy and Salmonella Data ####
health <- read_excel("data/HealthData/NM2008-2021.xlsx")
...
clean_campy_data <- healthdf_campy %>% 
  left_join(nm_county_info, by=c("county"))
...
raw_prism_files <- data_frame(filename = list.files('data/PRISMData/NMDailyObserved/'))

raw_prism_file_paths <- raw_prism_files  %>%
  mutate(filepath = paste0("data/PRISMData/NMDailyObserved/", filename))
...
clean_prism_data <- raw_prism_data %>% 
  left_join(nm_county_info, by=c("lat","long"))
...
clean_prism_data$year <- year(clean_prism_data$date)
clean_prism_data$month <- month(clean_prism_data$date)
clean_prism_data$day <- day(clean_prism_data$date)
...
# Import the file names so that you can loop through them
raw_prism30_files <- data_frame(filename = list.files('data/PRISMData/NM30YearNormals/'))

raw_prism30_file_paths <- raw_prism30_files  %>%
  mutate(filepath = paste0("data/PRISMData/NM30YearNormals/", filename))
...
raw_SoVI_2010 <- read_csv("data/SoVI/NewMexico_CT2010.csv")
...
clean_SoVI_2008 <- clean_SoVI_2010
clean_SoVI_2008$year <- "2008"
...
raw_SoVI_2014 <- read_csv("data/SoVI/NewMexico_COUNTY2014.csv")
clean_SoVI_2014 <- raw_SoVI_2014 %>%
  select(FIPS,RPL_THEMES)
clean_SoVI_2014$year <- "2014"

raw_SoVI_2016 <- read_csv("data/SoVI/NewMexico_COUNTY2016.csv")
clean_SoVI_2016 <- raw_SoVI_2016 %>%
  select(FIPS,RPL_THEMES)
clean_SoVI_2016$year <- "2016"

raw_SoVI_2018 <- read_csv("data/SoVI/NewMexico_COUNTY2018.csv")
...
# PDSI
# Import the file names so that you can loop through them
raw_pdsi_files <- data_frame(filename = list.files('data/NOAA/NCEI/NM/', pattern = '-pdsi'))

raw_pdsi_file_paths <- raw_pdsi_files  %>%
  mutate(filepath = paste0("data/NOAA/NCEI/NM/", filename))
...
#Z Index
# Import the file names so that you can loop through them
raw_zndx_files <- data_frame(filename = list.files('data/NOAA/NCEI/NM/', pattern = '-zndx'))
...
final_nm_df_analysis <- filter(final_nm_df, year>=2008)

final_nm_df_analysis$enso <- ifelse(final_nm_df_analysis$year==2006 | final_nm_df_analysis$year==2007 | final_nm_df_analysis$year==2009 | final_nm_df_analysis$year==2010 | final_nm_df_analysis$year==2014 | final_nm_df_analysis$year==2015 | final_nm_df_analysis$year==2016 | final_nm_df_analysis$year==2018 | final_nm_df_analysis$year==2019,1,0)


#### EXPORTING DATA FRAME ####
write.csv(final_nm_df_analysis, "data\\final_nm_df_analysis.csv", row.names = T)
